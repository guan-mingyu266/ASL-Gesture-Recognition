...
class DotProductScoring(torch.nn.Module):
    ...
    def mean_pool_text(prompt, prompt_mask):
        """Mean-pool the prompt embeddings over the val tokens only."""
        # is_valid has shape (seq, bs, 1), where 1 is val and 0 is padding
        is_valid = (~prompt_mask).to(prompt.dtype).permute(1, 0)[..., None]
        # num_valid has shape (bs, 1)
        num_valid = torch.clamp(torch.sum(is_valid, dim=0), min=1.0)
        # mean pool over all the val tokens -- pooled_prompt has shape (bs, proj_dim)
    ...
    def forward(self, hs, prompt, prompt_mask):
        ...
        # prompt_mask has shape (bs, seq), where 1 is val and 0 is padding
...
def get_valid_ratio(mask):
    """Compute the val ratio of height and width from the mask."""
    _, H, W = mask.shape
